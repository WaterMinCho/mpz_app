...
14. **Tailwind 예약어와 커스텀 토큰 충돌 주의**
15. **입양 문의 임시 전화 연결** — 구독 센터도 현재 전화 연결로 동작 중 (앱 내 입양 신청 절차는 임시 비활성). 복원 시 `list/animal/[id]/page.tsx`의 `handleAdoptionClick` 참고 — 커스텀 색상/변수명이 Tailwind 예약어(lg, xl, sm 등)와 겹치면 예기치 않은 스타일 오염 발생. font-size 지정 시 `text-[18px]` 같은 고정값 사용 권장
