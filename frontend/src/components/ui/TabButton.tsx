...
> = {
  ...
  variant3: {
    ...
    active: "text-bk font-bold h4 border-b-2 border-b-black flex-1",
...
export function TabButton({
  ...
  return (
    <Tabs value={value} onValueChange={onValueChange} className="w-full">
      <TabsList className="bg-transparent flex w-full overflow-x-auto scrollbar-hide whitespace-nowrap pb-2 min-w-max">
        {tabs.map((tab, index) => {
          const isActive = value === tab.value;
          // tab.value가 유효하지 않은 경우 index를 fallback으로 사용
          const tabKey =
