...
echo "🚀 Android Release APK 빌드 시작..."
...
echo "🏗️ Gradle Release APK 빌드 실행 (assembleRelease)"
cd "$ANDROID_DIR"
...
if [ -f "$APK_RELEASE_PATH" ]; then
	echo "✅ Release APK 빌드 성공: $APK_RELEASE_PATH"
	du -h "$APK_RELEASE_PATH" | awk '{print "📊 파일 크기:", $1}'
	# 편의상 프로젝트 루트로 복사
	PROJECT_ROOT="/Users/jhy20/mpz_fullstack"
	DEST_APK="$PROJECT_ROOT/MPZ-release.apk"
	cp "$APK_RELEASE_PATH" "$DEST_APK"
	echo "📁 사본 복사: $DEST_APK"
else
	echo "❌ Release APK를 찾을 수 없습니다. Gradle 로그를 확인하세요."
	exit 1
fi

echo ""
echo "📲 기기 설치 안내 (Release APK):"
echo "   adb install \"$APK_RELEASE_PATH\""
echo ""
echo "⚠️ 참고: 현재는 디버그 키로 서명되어 있습니다."
echo "   실제 배포를 위해서는 별도의 서명 키 설정이 필요합니다."
